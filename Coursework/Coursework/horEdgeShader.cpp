...
horEdgeShader::horEdgeShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"horEdgeShaderVS.cso", L"horEdgeShaderPS.cso");
}


horEdgeShader::~horEdgeShader()
{
	if (sampleState)
	{
		sampleState->Release();
...
void horEdgeShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void horEdgeShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, float height)
{
