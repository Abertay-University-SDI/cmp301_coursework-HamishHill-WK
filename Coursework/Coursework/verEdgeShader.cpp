...
verEdgeShader::verEdgeShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"verEdgeShaderVS.cso", L"verEdgeShaderPS.cso");
}


verEdgeShader::~verEdgeShader()
{
	if (sampleState)
	{
		sampleState->Release();
...
void verEdgeShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
